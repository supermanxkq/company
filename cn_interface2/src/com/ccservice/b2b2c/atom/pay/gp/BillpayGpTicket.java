 ...
 */
public class BillpayGpTicket extends AirSupper {

    public static void main(String[] args) throws Exception {
        Random r = new Random();
        JSONObject obj = new JSONObject();
        for (int i = 0; i < 1; i++) {
            String msg = new BillpayGpTicket(1739, "").pay(obj, 984.6d, "你好啊", 0);
     ...
     * @param OrderNumber
     */
    public BillpayGpTicket(long orderid, String OrderNumber) {
        if (orderid <= 0) {
    ...
    public String pay(JSONObject msgobj, double money, String goodsDesc, int ind) throws Exception {
        ...
        WriteLog.write("GpPay获取支付链接", ind + ":ordernumber:" + orderinfo.getOrdernumber() + ":分润参数：" + moneyShare);
    ...
    public String getRoyalty_parameters(long orderid, String OrderNumber, Orderinfo orderinfo, double Summoney) {
        ...
        if (list.size() > 0) {//已经创建不在创建
            ...
            if (orderinfo != null) {
                ...
                WriteLog.write("GpPay获取支付链接", "ordernumber:" + OrderNumber + "==>总金额：" + Summoney + ",采购金额："
                ...
                WriteLog.write("GpPay获取支付链接", "ordernumber:" + OrderNumber + "==>平台利润：" + platmoney);
                if (platmoney < 0) {
                    returnMsg = "0,";
                    returnnote = "代理商:46平台赔钱" + formatMoney(platmoney);
                    WriteLog.write("GpPay获取支付链接", "ordernumber:" + OrderNumber + "==>" + returnnote);
                }
                else {
                    try {
