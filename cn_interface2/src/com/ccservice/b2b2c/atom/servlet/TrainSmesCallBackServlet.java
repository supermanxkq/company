...
/**
 * Servlet implementation class TrainCallBackServlet
 */

public class TrainSmesCallBackServlet extends HttpServlet {
    public static void main(String[] args) {
        SendSms sms = SendSms.getInstance();
        String msg = "-1";
        Trainform form = new Trainform();
        form.setOrdernumber("T1512141914234909374");
        List<Trainorder> orders = Server.getInstance().getTrainService().findAllTrainorder(form, null);
        //List<Trainorder> orders = Server.getInstance().getTrainService().findTrainorderreport(form, null);
        List<Trainticket> tickets = Server.getInstance().getTrainService().findAllTrainTicket(form, null);
        //long l = 46426l;
        ...
        if (trainorder != null) {
            for (Trainpassenger passenger : trainorder.getPassengers()) {
                for (Trainticket ticket : passenger.getTraintickets()) {
                    //System.out.println(trainorder.getPassengers().get(0).getTraintickets().get(0).getId());
                    //System.out.println(trainorder.getPassengers().get(0).getTraintickets().get(0).getDeparture());
                    System.out.println(ticket.getId());
                }
            }
            sms.sendSMS(msg, "18363852969");
        }
    }
     ...
     */
    public TrainSmesCallBackServlet() {
        super();
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        String msg = "-1";
        //        String issuccess = req.getParameter("issuccess");//处理业务数据
        //        String transactionid = req.getParameter("transactionid");//处理业务数据
        //        System.out.println(issuccess + "------transactionid" + transactionid);
        //接收参数
        ...
        WriteLog.write("火车票短信", "出票成功：JsonStr=" + JsonStr);
