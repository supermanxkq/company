...
public class TuNiuTrainAccountDeleteServlet extends HttpServlet {
        ...
        try {
            ...
            if (ElongHotelInterfaceUtil.StringIsNull(reqString)) {
                ...
                if (data.equals("")) {
                    ...
                    JSONObject jsonarray = JSON.parseObject(new String(b));
                    String trainAccount = jsonarray.getString("trainAccount");
                    String pass = jsonarray.getString("pass");
                    String ids = jsonarray.getString("ids");
                    JSONObject jresult = new JSONObject();
                    jresult.put("trainAccount", trainAccount);
                    jresult.put("pass", pass);
                    jresult.put("id", ids);
                    result = account.Accountdelete(jresult).toJSONString();
                }
            }
        }
        catch (Exception e) {
