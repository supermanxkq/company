 ...
 *
 */
public class TongChengTrainInsure {
    private static TongChengTrainInsure tcti = null;

    private TongChengTrainInsure() {

    }

    public static TongChengTrainInsure getTongChengTrainInsure() {
        if (tcti == null) {
            tcti = new TongChengTrainInsure();
        }
        return tcti;
    }
    ...
    public Trainorder insuranceSynchronous(Trainorder trainorder) {
        for (Trainpassenger trainpassenger : trainorder.getPassengers()) {
            for (Trainticket trainticket : trainpassenger.getTraintickets()) {
                //                System.out.println("trainticket.getId():" + trainticket.getId() + "trainticket.getInsurorigprice():"
                //                        + trainticket.getInsurorigprice() + ",trainticket.getRealinsureno():"
                //                        + trainticket.getRealinsureno());
                //                System.out.println(trainticket.getInsurorigprice() > 0);
                //                System.out.println(trainticket.getRealinsureno() == null);
                //                System.out.println("".equals(trainticket.getRealinsureno()));
                        ...
                        && (trainticket.getRealinsureno() == null || "".equals(trainticket.getRealinsureno())))//票里的保险价钱大于0说明客户需要投保
                {
                    ...
                    Insuruser insuruser = insureTrain(trainpassenger.getIdtype() + "", trainpassenger.getIdnumber(),
                            ...
                            trainorder.getContacttel(), trainorder.getAgentid(), r1, trainorder.getId(),
                            trainorder.getInsureadreess(), trainorder.getOrdernumber(), trainorder.getCreatetime(),
                            trainorder.getQunarOrdernumber(), "0", trainorder.getId());
                    if (insuruser != null && insuruser.getPolicyno() != null && !"".equals(insuruser.getPolicyno())) {
                        ...
                        WriteLog.write("TongCheng_toubao", trainorder.getId() + ":" + trainpassenger.getName() + ":"
    ...
    public Insuruser insureTrain(String idtype, String idno, String traindate, String birthday,
            ...
            String adds, String ordernum, Timestamp createdate, String qunarordernum, String flag, long orderid) {
        WriteLog.write("同城保险接口_投保", r1 + ":idtype:" + idtype + ":idno:" + idno + ":traindate:" + traindate
    ...
    public boolean tuibao(int trainorderid, String msg, String Insureno) {
        ...
        try {
            ...
            WriteLog.write("同城保险接口_退保", "trainorder" + trainorderid + " Insureno" + Insureno + "jsonObject"
                    + jsonObject);
            return false;
        }
        catch (Exception e) {
    ...
    public static void testcancelinsure(Trainorder trainorder, Trainticket trainticket) {
        ...
        if (trainticket.getRealinsureno() != null) {
            try {
                b = TongChengTrainInsure.getTongChengTrainInsure().tuibao(Integer.parseInt(trainorder.getId() + ""),
                        "客户退票", trainticket.getRealinsureno());
    ...
    public static void testinsure() {
        ...
        Trainorder trainorder = Server.getInstance().getTrainService().findTrainorder(46481);
        trainorder = TongChengTrainInsure.getTongChengTrainInsure().insuranceSynchronous(trainorder);
