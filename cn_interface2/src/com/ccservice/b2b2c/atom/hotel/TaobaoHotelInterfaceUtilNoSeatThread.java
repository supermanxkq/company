...
public class TaobaoHotelInterfaceUtilNoSeatThread extends Thread {

    private Trainticket trainticket;

    private Trainorder trainorder;

    private boolean acquiringresult;

    public TaobaoHotelInterfaceUtilNoSeatThread(boolean acquiringresult, Trainticket trainticket, Trainorder trainorder) {
        this.acquiringresult = acquiringresult;
    ...
    public void run() {
        if (trainticket != null) {
            ...
            for (Trainpassenger trainpassenger : this.trainorder.getPassengers()) {
                for (Trainticket tt : trainpassenger.getTraintickets()) {
                    String sql = " [sp_T_TRAINTICKET_UPDATE] @ID=" + tt.getId() + ", @seatType='" + zwtype + "'";
                    WriteLog.write("无座转换", this.trainorder.getId() + "--->" + tt.getId() + "--->sql--->" + sql);
                    try {
                        Server.getInstance().getSystemService().findMapResultByProcedure(sql);
                    }
                    catch (Exception e) {
                        WriteLog.write("无座转换_ERROR", this.trainorder.getId() + "--->sql--->" + sql);
                        ExceptionUtil.writelogByException("无座转换_ERROR", e);
                    }
                }
            }
            activeMQroordering(this.acquiringresult, this.trainorder);
        }
        else {
     ...
     * @author chendong
     */
    public void activeMQroordering(boolean acquiringresult, Trainorder trainorder) {
        WriteLog.write("无座转换", "id--->" + trainorder.getId());
        if (acquiringresult) {
            ...
            try {
                ActiveMQUtil.sendMessage(url, QUEUE_NAME, trainorder.getId() + "");
            }
            catch (Exception e) {
    ...
    private JSONObject queryLowestSeattype(String orderNumber, String fromDate, String fromStation, String toStation,
        ...
        if (flag) {
            String zwtype = getSurplusTrainTicket(trainoJson, trainNum);
    ...
    private String getSurplusTrainTicket(JSONObject trainJson, String trainNum) {
        ...
        if (trainJson != null && !trainJson.isEmpty()) {
            ...
            if (!"--".equals(qtxb_num)) {
                if (traintype[0].subSequence(0, 1).equals("K") || traintype[0].subSequence(0, 1).equals("Z")
                        || traintype[0].subSequence(0, 1).equals("T")) {
                    zwtype = "硬座";
                }
                else if (traintype[0].subSequence(0, 1).equals("D") || traintype[0].subSequence(0, 1).equals("G")
                        || traintype[0].subSequence(0, 1).equals("C")) {
                    zwtype = "二等座";
                }
            }
