...
import com.ccservice.b2b2c.atom.interticket.HttpsURLConnectionPost;
...
public class ElongHotelInterfaceUtil {
    /**中转地址*/
    public static String getMiddleUrl(String elmethod, String req, String httpType) throws Exception {
        String ip = Server.getInstance().getElongMiddleUrl();
        return "http://" + ip + "/cn_interface/ElongHotel.jsp?api=" + elmethod + "&httpType=" + httpType + "&req="
                + URLEncoder.encode(req, "utf-8");
    }

    /**艺龙地址 httpType为http或https*/
    public static String getELUrl(String httpType) {
        //测试
        //return httpType + "://api.test.lohoo.com/rest?";
        //正式
        return httpType + "://api.elong.com/rest?";
    }

    /**艺龙用户*/
    public static String getElUser() {
        //测试
        //return "Agent07";
        //正式
        //return "f584588cbbc70eb72d2bf95b1867e3cc";
        return Server.getInstance().getElongUserName();
    }

    /**艺龙appKey*/
    public static String getELAppKey() {
        //测试
        //return "70d4f84b99209a631a3de51978d6a4ee";
        //正式
        //return "bac3d8724497839cb05dcbddacd5edfd";
        return Server.getInstance().getElongAppKey();
    }

    /**艺龙secretKey*/
    public static String getELSecretKey() {
        //测试
        //return "0245e52b8e2a3518b1a01b7694752528";
        //正式
        //return "85e433c71cf1482b22eee36e526a96f0";
        return Server.getInstance().getElongSecretKey();
    }

    /**艺龙请求格式*/
    public static String getELFormat() {
        return "json";
    }

    /**艺龙API版本*/
    public static String getELVersion() {
        return "1.02";
    }

    /**艺龙酒店详情访问地址*/
    public static String getELHotelDetailUrl(String hotelId) {
        return "http://api.elong.com/xml/v2.0/hotel/cn/" + hotelId.substring(hotelId.length() - 2, hotelId.length())
                + "/" + hotelId + ".xml";
    }
     ...
     * @param elmethod 请求方法
     * @param req 请求参数
     * @param httpType 固定值：http或https
     */
    public static String postEl(String elmethod, String req, String httpType) throws Exception {
        //API版本、语言
        JSONObject conditionjson = new JSONObject();
        conditionjson.put("Version", getELVersion());
        conditionjson.put("Local", "zh_CN");
        conditionjson.put("Request", req);
        String data = conditionjson.toString();
        conditionjson = new JSONObject();
        conditionjson.put("Condition", data);
        //时间戳
        long timestamp = System.currentTimeMillis() / 1000;
        //MD5
        String signature = MD5(timestamp + MD5(data + getELAppKey()) + getELSecretKey()).toLowerCase();
        //URL
        String elurl = getELUrl(httpType) + "format=" + getELFormat() + "&method=" + elmethod + "&signature="
                + signature + "&user=" + getElUser() + "&timestamp=" + timestamp + "&data="
                + URLEncoder.encode(data, "utf-8");
        //艺龙返回数据
        if ("http".equals(httpType)) {
            return SendPostandGet.submitPost(elurl, "", "utf-8").toString();
        }
        else if ("https".equals(httpType)) {
            return new HttpsURLConnectionPost().submitPost(elurl, "", "utf-8").toString();
        }
        else {
            throw new Exception("Httptype should be http or https.");
        }
    }

    /**艺龙信用卡号DES加密*/
    public static String DES(String creditnumber) throws Exception {
        if (StringIsNull(creditnumber)) {
            throw new Exception("信用卡号为空.");
        }
        String key = getELAppKey();
