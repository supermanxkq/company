...
public class TrainticketRefundHandle implements RefundHandle {
    Log log = LogFactory.getLog(TrainticketRefundHandle.class);

    @Override
    public void refundedHandle(boolean success, long orderid, String batchno) {
        WriteLog.write("退票退款", success + "--->" + orderid + "--->" + batchno);
        try {
            ...
            if (success) {
                rc.setContent("退票成功。退款方式：" + Paymentmethod.getPaymethod(Paymentmethod.EBANKPAY) + ",退款批次号：" + batchno);
                Server.getInstance()
                        .getSystemService()
                        .excuteSysconfigBySql(
                                "UPDATE T_TRAINTICKET SET C_STATUS=" + Trainticket.REFUNDED + " WHERE ID="
                                        + ticket.getId());
            }
            else {
                rc.setContent("退票退款失败");
                Server.getInstance()
                        .getSystemService()
                        .excuteSysconfigBySql(
                                "UPDATE T_TRAINTICKET SET C_STATUS=" + Trainticket.REFUNDFAIL + " WHERE ID="
                                        + ticket.getId());
            }
            ...
            WriteLog.write("退票退款_Exception", orderid + "");
            ExceptionUtil.writelogByException("退票退款_Exception", e);
        }
    ...
    private void getTrainRefundAlipayMethod(long ticketid) {
        String sql = " [sp_TrainRefundAlipayMethod_Delete] @Ticketid=" + ticketid;
