...
public class AirTicketRefundHandle implements RefundHandle {

	@Override
	public void refundedHandle(boolean success, long orderid, String batchno) {

		Orderchange change = Server.getInstance().getB2BAirticketService()
				.findOrderchange(orderid);
		if (change.getChangestate() == 6) {
			...
			String sql = "UPDATE T_ORDERCHANGE SET C_CHANGESTATE=" + changestue
			...
			try {
				...
				Server.getInstance().getRateService().ticketrefundNotify(
						orderid);
